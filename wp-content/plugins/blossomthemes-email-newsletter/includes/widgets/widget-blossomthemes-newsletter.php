...
class BlossomThemes_Email_Newsletter_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		?>
				...
				<?php
				if ( isset( $blossomthemes_email_newsletter_setting['appearance']['newsletter-bg-option'] ) && $blossomthemes_email_newsletter_setting['appearance']['newsletter-bg-option'] == 'image' ) {
					echo ' bg-img', $overlay; }
				?>
				" id="boxes-<?php echo esc_attr( $atts['id'] ); ?>" style="background: <?php echo esc_attr( $rrsb_bg ); ?>; color:<?php echo esc_attr( $rrsb_fc ); ?>;">
					
					<?php
					$inner_wrap = apply_filters( 'bt_newsletter_widget_inner_wrap_display', false );
					if ( $inner_wrap ) {
				...
				$bten_settings = get_option( 'blossomthemes_email_newsletter_settings', true );
                    ...
                    $(document).ready(function() { 
                        $(document).on("submit","form#blossomthemes-email-newsletter-' . esc_attr( $atts['id'] ) . '", function(e){
                        e.preventDefault();
                        $(".subscribe-submit-' . esc_attr( $atts['id'] ) . '").attr("disabled", "disabled" );
                        var email = $(".subscribe-email-' . esc_attr( $atts['id'] ) . '").val();
                        var fname = $(".subscribe-fname-' . esc_attr( $atts['id'] ) . '").val();
                        var sid = ' . esc_attr( $atts['id'] ) . ';
                        var nonce = $("#bten_subscription_nonce_' . esc_attr( $atts['id'] ) . '").val();
                            $.ajax({
                                type : "post",
                                dataType : "json",
                                ...
                                beforeSend: function(){
                                    $("#loader-' . esc_attr( $atts['id'] ) . '").fadeIn(500);
                                },
				...
				if ( $option == 'text' ) {
					$ajax .= '$("#bten-response-' . esc_attr( $atts['id'] ) . ' span").html(response.message);$("#bten-response-' . esc_attr( $atts['id'] ) . '").fadeIn("slow").delay("3000").fadeOut("3000",function(){
                                            $(".subscribe-submit-' . esc_attr( $atts['id'] ) . '").removeAttr("disabled", "disabled" );
                                            $("form#blossomthemes-email-newsletter-' . esc_attr( $atts['id'] ) . '").find("input[type=text]").val("");
                                        });';
				} else {
					...
					$ajax         .= 'window.location.href = "' . esc_url( $url ) . '"';
								...
								$ajax .= '}
                                    ...
                                    $("#bten-response-' . esc_attr( $atts['id'] ) . ' span").html(response.message);$("#bten-response-' . esc_attr( $atts['id'] ) . '").fadeIn("slow").delay("3000").fadeOut("3000",function(){
                                            $(".subscribe-submit-' . esc_attr( $atts['id'] ) . '").removeAttr("disabled", "disabled" );
                                            $("form#blossomthemes-email-newsletter-' . esc_attr( $atts['id'] ) . '").find("input[type=text]").val("");
                                        });
                                    }
                                ...
                                complete: function(){
                                    $("#loader-' . esc_attr( $atts['id'] ) . '").fadeOut(500);             
                                } 
                            });  
                        });
                    });
	...
	public function form( $instance ) {
		...
		$arg         = array(
			'posts_per_page' => -1,
			'post_type'      => array( 'subscribe-form' ),
		);
		$posts       = get_posts( $arg );
		...
		<p>
			...
			<span id="footer-thankyou">
				...
				$bold      = '<b>';
				$boldclose = '</b>';
				echo sprintf( __( 'To create a new newsletter form, go to %1$sBlossomThemes Email Newsletters > Add New%2$s', 'blossomthemes-email-newsletter' ), $bold, $boldclose );
				?>
			</span>
		</p>
		<p>
